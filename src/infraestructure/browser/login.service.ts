...
async function fillLoginForm(page: Page, credentials: LoginCredentials): Promise<void> {
  ...
  try {
    ...
    // Log the available options to see what values are actually accepted
    const optionsHTML = await page.locator(loginSelectors.docType).innerHTML();
    logger.error(`[Login] No se encontró el tipo de documento '${credentials.documentType}'. Opciones disponibles en el HTML:\n${optionsHTML}`);
    throw error;
  }
